...
const ServiceCard = ({ icon, title }) => {    
  ...
  const { ref: myref, inView, entry } = useInView(options); 
    ...
    return (
      <>
        <div
            className={`${inView?'services-zoom-in':null} hover:scale-110 transition ease-in-out delay-100 duration-300 relative sm:m-10 m-5 grid h-[15rem] w-full max-w-[15rem] flex-col items-end justify-center overflow-hidden rounded-xl bg-white bg-clip-border text-center text-gray-700`} ref={myref}>
            <div
